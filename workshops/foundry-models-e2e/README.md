...
| 6 | [Fine-tune `gpt-4.1` for policy](./06-finetune.md) | SDK + Skill | Policy 0.28 → ~0.94, cost ÷50 vs. frontier |
...
│   ├── policy-ft-train.jsonl      ← 24 chat-format rows for fine-tune
│   ├── policy-ft-val.jsonl        ← 6 held-out validation rows
...
└── code/
    ...
    └── s06_finetune_policy.py    ← fine-tune gpt-4.1 on policy QA
```
