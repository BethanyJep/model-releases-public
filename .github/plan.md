...
6. **Recently added** — single table of latest 5 capsules with columns
   Model / Release date (linked to blog) / Expires (⚠️ when within 60
   days) / Pricing. Regenerated by the `refresh-recent-activity` skill
...
- **`refresh-recent-activity`** — regenerate the **Recently added**
  ...
  markers at the top of the repo README. Columns: Model (→ capsule),
  Release date (→ announcement URL), Expires (⚠️-bolded when within 60
  days). Top 5 capsules by release-date folder name. Runs after
  `add-capsule` and on a monthly cadence.
