...
style: |
  ...
  h1 { font-size: 52px; line-height: 1.1; margin: 0 0 24px; }
...
═══════════════════════════════════════════════════════════════════════
  ...
  Speaker notes live inside <!-- ... --> blocks like this one.
  Custom slide styles via "<!-- _class: name -->" directive.
═══════════════════════════════════════════════════════════════════════
-->
...
**Senior engineer**
Contoso San Diego office

Needs to be in **Berlin Tuesday morning** for an offsite that runs through Thursday.

She messages the agent at 11 PM:
...
# v1 — one model, all jobs.

The version of this we all ship first.

---

# v1 architecture — everything on `gpt-5.4`

```python
# This is the entire architecture
client.chat.completions.create(
    model="planner-gpt54",          # gpt-5.4
...
<span class="pill">v1 — the CFO math</span>
...
<div class="stat-row">
  <div class="stat"><div class="label">Project</div><div class="val" style="font-size:24px;line-height:1.3">Workspace that ties everything together</div></div>
  <div class="stat"><div class="label">Models + Endpoints</div><div class="val" style="font-size:24px;line-height:1.3">What's deployed, where, at what SKU</div></div>
  <div class="stat"><div class="label">Playground</div><div class="val" style="font-size:24px;line-height:1.3">Try a prompt against a deployment</div></div>
</div>
<div class="stat-row">
  <div class="stat"><div class="label">Evaluation</div><div class="val" style="font-size:24px;line-height:1.3">Curated · batch · continuous · red team</div></div>
  <div class="stat"><div class="label">Fine-tuning</div><div class="val" style="font-size:24px;line-height:1.3">Train, deploy, version, compare</div></div>
  <div class="stat"><div class="label">Tracing</div><div class="val" style="font-size:24px;line-height:1.3">Per-call latency, tokens, errors</div></div>
</div>
...
<div class="stat-row">
  <div class="stat"><div class="label">Start</div><div class="val" style="font-size:28px">🌐 Portal</div><div class="note" style="font-size:16px">create project · catalog · playground baseline</div></div>
  <div class="stat"><div class="label">Build</div><div class="val" style="font-size:28px">💻 VS Code + SDK</div><div class="note" style="font-size:16px">agent · router · eval · fine-tune</div></div>
  <div class="stat"><div class="label">Close</div><div class="val" style="font-size:28px">🌐 Portal</div><div class="note" style="font-size:16px">evals · red team · agent versions</div></div>
</div>
...
<span class="note">Foundry catalog filter: *Sold directly by Azure · East US 2 · Deployable*</span>

| # | Job | Pick | Why |
|---|---|---|---|
| Route | intent classify | **`gpt-5.4-nano`** | Sub-300ms · plenty smart for a 3-way classify |
| Vision | extract receipt | **`gpt-5.4-mini`** | Vision support · frontier overkill |
| Policy | *"can I expense X?"* | **`gpt-5.4-mini`** → fine-tune | Cheap base + fine-tunable |
| Plan | multi-step + tools | **`gpt-5.4`** | Frontier earns its keep here |
| Translate | EN ↔ DE | **`gpt-5.4-mini`** (reuse) | Same deployment as vision |
...
<!--
Naomi: "I asked the Foundry Skill — 'pick Azure Direct models for these five tasks in East US 2.' It returned this table in 8 seconds. I read it, agreed, deployed."
-->
...
```text
> List Azure Direct models in East US 2 matching gpt-5.4*.
> For each: exact version · max tokens · supports fine-tuning ·
> input/output price per 1K.

> Now deploy:
>   router-nano        (gpt-5.4-nano,  TPM 10K)
>   mini-vision        (gpt-5.4-mini,  TPM 20K)
>   policy-mini-base   (gpt-5.4-mini,  TPM 20K)
```
...
```
planner-gpt54        ← gpt-5.4
router-nano          ← gpt-5.4-nano
mini-vision          ← gpt-5.4-mini
policy-mini-base     ← gpt-5.4-mini
policy-mini-ft       ← gpt-5.4-mini (fine-tuned, coming in Part 9)
```
...
# v2 — route to the right model per task
...
itinerary = planner.run(user_msg, ...)    # planner-gpt54 + tools
```
...
**Two evaluators**
deterministic schema/constraint check
...
# Demo — fine-tune `gpt-5.4-mini` for policy
...
<div class="stat-row">
  <div class="stat bad">
    <div class="label">gpt-5.4-mini base</div>
    <div class="val">0.81</div>
    <div class="note" style="font-size:18px">$0.0008 / answer</div>
  </div>
  <div class="stat good">
    <div class="label">policy-mini-ft</div>
    <div class="val">0.94</div>
    <div class="note" style="font-size:18px">$0.0010 / answer</div>
  </div>
  <div class="stat bad">
    <div class="label">gpt-5.4 frontier</div>
    <div class="val">0.93</div>
    <div class="note" style="font-size:18px">$0.045 / answer</div>
  </div>
</div>
...
|---|---|---|---|---|
| **v1** | `gpt-5.4` does everything | 0.61 | $0.108 | 12.3s |
...
<span class="pill">v1 → v3 · the deltas</span>

<div class="stat-row">
  <div class="stat good">
    <div class="label">Quality</div>
    <div class="val">+54%</div>
    <div class="note" style="font-size:18px">0.61 → 0.94</div>
  </div>
  <div class="stat good">
    <div class="label">Cost</div>
    <div class="val">–74%</div>
    <div class="note" style="font-size:18px">$0.108 → $0.028</div>
  </div>
  <div class="stat good">
    <div class="label">Latency</div>
    <div class="val">–38%</div>
    <div class="note" style="font-size:18px">12.3s → 7.6s</div>
  </div>
</div>
...
**All on Microsoft Foundry. East US 2. 7-day build. Toy-scale data.**
...
**Repo:** `nitya/model-releases-FORK` → `plannning/`
**Plan:** *PLAN.md* (full speaker script + trainer guide)
**Hands-on:** *tutorial/README.md* (Steps 0–8, ~7 days of evenings)
**Code:** *tutorial/code/* (`s02_…` through `s06_…`)

<br>

**Region:** East US 2  ·  **Models:** `gpt-5.4` family (Azure Direct)
